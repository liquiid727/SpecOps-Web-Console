...
description: Use when a Go backend task touches interfaces or transport-layer request and response models, handler success payloads, Swagger request or response annotations, time-field serialization, or SafeTime and response wrapping rules.
version: 1.0.0
category: governance
tags:
  - api
...
这是仓库级横切治理 skill，用来统一 `interfaces/api` 的 request/response 契约、`SafeTime` 使用方式，以及 handler/Swagger 的稳定输出规则。
...
- 审查时间字段是继续裸 `time.Time`，还是要切换成 `*time.Time` / `SafeTime`
...
- 领域 / 应用 / model 层的 optional time 优先使用 `*time.Time`
...
- 判断某个时间字段是否需要 `SafeTime` / `NewSafeTimePtr`
