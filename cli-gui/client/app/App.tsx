...
export function App() {
  ...
  const { overlay, pendingDelete, pickerBusy, newSessionDefaultMode, questDraftActive, setOverlay, setPendingDelete, setPickerBusy, setNewSessionDefaultMode, setQuestDraftActive } = useUiStore();
  ...
  // Quest Home 一次提交创建流：Chat-first（issue-066）——CHAT_ENABLED 时默认创建 chat 会话（走 AgentBackend 结构化通道），
  // Profile 不支持 headless 时服务端显式降级 terminal 并提示原因；创建后 start-and-send 首轮，随后进入该会话
  ...
  const quickCreateSession = useCallback(async (input: { content: string; workspaceId: string; profileId: string; model?: string }) => {
    ...
    updatePreferences({ currentView: "chat" });
    await refresh();
  }, [feedback, preferences.modelPreferences.lastUsedModel, refresh, runtime.sessions, setQuestDraftActive, t, updatePreferences]);
  ...
  return <div className="qoder-app">
    ...
    <main className={`qoder-body ${preferences.navigatorOpen ? "sidebar-open" : ""} ${showRightPanel && preferences.inspectorOpen ? "right-open" : ""}`}>
      {preferences.navigatorOpen && <Sidebar questGroups={questGroups} chatGroups={chatGroups} workspaces={state.workspaces} activeSessionId={activeSessionId} activeTurns={activeTurns} currentView={preferences.currentView} grouping={preferences.sessionGrouping} filter={preferences.sessionFilter} readonly={readonly} openFolderBusy={pickerBusy} questDraftActive={questDraftActive} onViewChange={handleViewChange} onNewQuest={startQuestDraft} onSelectSession={selectSession} onGroupingChange={(sessionGrouping) => updatePreferences({ sessionGrouping })} onFilterChange={(sessionFilter) => updatePreferences({ sessionFilter })} onReorder={reorderSessions} onOpenFolder={openFolder} onOpenSettings={() => setOverlay("settings")} onRename={(session) => { setActiveSessionId(session.id); setOverlay("rename"); }} onPin={(session) => void runAction(() => runtime.sessions.pinSession(session.id, !session.pinned, session.revision ?? 1), false)} onComplete={(session) => { setActiveSessionId(session.id); if (session.organizationStatus === "completed") void runAction(() => runtime.sessions.restoreSession(session.id, session.revision ?? 1), false); else setOverlay("complete-session"); }} onArchive={(session) => { setActiveSessionId(session.id); if (session.organizationStatus === "archived") void runAction(() => runtime.sessions.restoreSession(session.id, session.revision ?? 1), false); else setOverlay("archive-session"); }} onFork={(session) => { setActiveSessionId(session.id); setOverlay("fork-session"); }} onDelete={(session) => { setActiveSessionId(session.id); setOverlay("delete-session"); }} onClose={() => updatePreferences({ navigatorOpen: false })} />}
