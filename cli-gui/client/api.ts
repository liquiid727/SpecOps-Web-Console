...
export const api = {
  ...
  createSession: (input: { name: string; workspaceId: string; profileId: string; confirmed: boolean; start?: boolean; terminal?: { cols: number; rows: number } }) => request<SessionWithCompatibilityStatus & { session?: SessionWithCompatibilityStatus; capabilities?: CliProfileCapabilities }>("/api/sessions", { method: "POST", body: JSON.stringify({ start: input.start ?? true, ...input }) }),
