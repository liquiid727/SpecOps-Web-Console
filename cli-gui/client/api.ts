...
export const api = {
  ...
  createProvider: (input: { id: string; name: string; protocol: string; baseUrl: string; models?: string[]; supportedEngineIds?: string[] }) => request<ProviderListResponse>("/api/providers", { method: "POST", body: JSON.stringify(input) }),
  updateProvider: (id: string, input: Partial<{ name: string; protocol: string; baseUrl: string; models: string[]; supportedEngineIds: string[]; enabled: boolean }>) => request<ProviderListResponse>(`/api/providers/${id}`, { method: "PATCH", body: JSON.stringify(input) }),
