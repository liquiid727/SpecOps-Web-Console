import type { ApiErrorCode, ApiErrorResponse, DowngradeReason, EngineReadinessResponse, ExecutionTaskListResponse, ExecutionTaskResponse, FilePreview, FileTreePage, GitDiffResponse, GitStatusResponse, LanguageSummaryResponse, ModelDeploymentConfig, ModelDeploymentSummary, ModelProviderSummary, ModelRoutePreviewRequest, PickWorkspaceResponse, PriorityModelRoute, ProviderCredentialResponse, ProviderListResponse, ResolvedRoute, RouteListResponse, SendMessageRequest, SendMessageResponse, SessionWithCompatibilityStatus, StateResponse, TranscriptPage } from "../shared/types";
...
export const api = {
  ...
  updateSessionRoute: (id: string, modelRouteId: string | null, expectedRevision: number) => request<SessionWithCompatibilityStatus>(`/api/sessions/${id}`, { method: "PATCH", body: JSON.stringify({ modelRouteId, expectedRevision }) }),
  ...
  providers: (signal?: AbortSignal) => request<ProviderListResponse>("/api/providers", { signal }),
  createProvider: (input: { id: string; name: string; protocol: string; baseUrl: string; credentialRef?: string; models?: string[]; supportedEngineIds?: string[] }) => request<ProviderListResponse>("/api/providers", { method: "POST", body: JSON.stringify(input) }),
  updateProvider: (id: string, input: Partial<{ name: string; protocol: string; baseUrl: string; credentialRef: string; models: string[]; supportedEngineIds: string[]; enabled: boolean }>) => request<ProviderListResponse>(`/api/providers/${id}`, { method: "PATCH", body: JSON.stringify(input) }),
  deleteProvider: (id: string) => request<void>(`/api/providers/${id}`, { method: "DELETE", body: "{}" }),
  setProviderCredential: (id: string, secret: string) => request<ProviderCredentialResponse>(`/api/providers/${id}/credential`, { method: "PUT", body: JSON.stringify({ secret }) }),
  deleteProviderCredential: (id: string) => request<ProviderCredentialResponse>(`/api/providers/${id}/credential`, { method: "DELETE", body: "{}" }),
  modelDeployments: (signal?: AbortSignal) => request<{ deployments: ModelDeploymentSummary[] }>("/api/model-deployments", { signal }),
  createModelDeployment: (input: Omit<ModelDeploymentConfig, "createdAt" | "updatedAt" | "archivedAt">) => request<{ deployments: ModelDeploymentSummary[] }>("/api/model-deployments", { method: "POST", body: JSON.stringify(input) }),
  updateModelDeployment: (id: string, input: Partial<ModelDeploymentConfig>) => request<{ deployments: ModelDeploymentSummary[] }>(`/api/model-deployments/${id}`, { method: "PATCH", body: JSON.stringify(input) }),
  deleteModelDeployment: (id: string) => request<void>(`/api/model-deployments/${id}`, { method: "DELETE", body: "{}" }),
  modelRoutes: (signal?: AbortSignal) => request<RouteListResponse>("/api/model-routes", { signal }),
  createModelRoute: (input: Omit<PriorityModelRoute, "createdAt" | "updatedAt" | "archivedAt">) => request<RouteListResponse>("/api/model-routes", { method: "POST", body: JSON.stringify(input) }),
  updateModelRoute: (id: string, input: Partial<PriorityModelRoute>) => request<RouteListResponse>(`/api/model-routes/${id}`, { method: "PATCH", body: JSON.stringify(input) }),
  deleteModelRoute: (id: string) => request<void>(`/api/model-routes/${id}`, { method: "DELETE", body: "{}" }),
  previewModelRoute: (input: ModelRoutePreviewRequest) => request<{ resolvedRoute: ResolvedRoute; deployments: ModelDeploymentSummary[] }>(`/api/model-routes/${encodeURIComponent(input.routeId ?? "inherit")}/resolve`, { method: "POST", body: JSON.stringify(input) }),
  resolveSessionModelRoute: (id: string, fixedDeploymentId?: string) => request<{ resolvedRoute: ResolvedRoute }>(`/api/sessions/${id}/model-route/resolve`, { method: "POST", body: JSON.stringify(fixedDeploymentId ? { fixedDeploymentId } : {}) }),
  executionTasks: (sessionId: string, after?: string, limit?: number, signal?: AbortSignal) => request<ExecutionTaskListResponse>(`/api/sessions/${sessionId}/execution-tasks${after || limit ? `?${new URLSearchParams({ ...(after ? { after } : {}), ...(limit ? { limit: String(limit) } : {}) })}` : ""}`, { signal }),
  executionTask: (taskId: string, signal?: AbortSignal) => request<ExecutionTaskResponse>(`/api/execution-tasks/${taskId}`, { signal }),
  confirmExecutionRetry: (taskId: string, input: { expectedRevision: number; confirmationToken: string; inputSha256: string }) => request<ExecutionTaskResponse>(`/api/execution-tasks/${taskId}/confirm-retry`, { method: "POST", body: JSON.stringify(input) }),
  cancelExecution: (taskId: string, expectedRevision?: number) => request<ExecutionTaskResponse>(`/api/execution-tasks/${taskId}/cancel`, { method: "POST", body: JSON.stringify(expectedRevision === undefined ? {} : { expectedRevision }) })
};
