...
export function PreviewTab({ workspace, initialPath }: { workspace?: Workspace; initialPath: string }) {
  ...
  return <div className="preview-tab">
    <div className="preview-form"><TextField aria-label={t("filePath")} placeholder="README.md" value={path} onChange={(event) => setPath(event.target.value)} /><Button variant="secondary" className="secondary-button" onClick={() => void load()}>{t("preview")}</Button></div>
...
export function GitTab({ workspace }: { workspace?: Workspace }) {
  ...
  return <div className="inspector-list">
    <div className="inspector-tab-toolbar"><span className="eyebrow">{t("gitState")}</span><IconButton icon="refresh" onClick={() => setReload((value) => value + 1)} label={t("refresh")} title={t("refresh")} /></div>
...
export function InspectorState({ text, error = false, onRetry }: { text: string; error?: boolean; onRetry?: () => void }) {
  const { t } = useI18n();
  return <div className={`inspector-state ${error ? "error" : ""}`} role={error ? "status" : undefined}><span>{text}</span>{onRetry && <Button variant="secondary" className="secondary-button" onClick={onRetry}>{t("retry")}</Button>}</div>;
}
