---
name: loop-it
description: "Automated Spec-traceable Issue loop with checkpoint/resume: dependency-aware execution, implementation, required independent verification evidence, review, notes, and gated shipping. Use for batches of approved implementation or verification Issues; never ship when the linked Test Spec is stale or blocking evidence is missing."
allowed-tools:
  - Bash(gh:*)
  - Bash(git:*)
  - Bash(cat:*)
...
Fetch open GitHub Issues, preserve their implementation or verification track, resolve dependency order, and move each through implementation or test execution, evidence validation, `/review-it`, `/note-it`, and `/ship-it`.
...
Update state: `pending` → `skipped` or `pending` → `blocked`, write checkpoint, run **3i Branch Cleanup**, proceed to next issue.
...
### 3d. Independent Verification Evidence Gate

When the Issue references a Feature Spec:

1. Resolve the exact `spec_id` and `spec_version`.
2. Require an approved Test Spec for that source version when independent verification applies.
3. Run or collect the verification Issue outputs required by the Test Spec.
4. Reject stale Test Specs, version-mismatched results, failed P0/P1 evidence, and missing blocking artifacts.
5. Mark the Issue `blocked` instead of shipping when the gate is incomplete.

Implementation-coupled unit tests do not replace independent API, scenario, UI/E2E, performance, load, stress, concurrency, or security evidence required by the Test Spec.
...
Update state: `in_progress` → `failed`, write checkpoint, run **3i Branch Cleanup**, proceed to next issue.

### 3e. Review with /review-it
...
Update state: `in_progress` → `failed`, `phase: "ship"`, write checkpoint, run **3i Branch Cleanup**, proceed to next issue.
...
| evidence_blocked | Test Spec 失效、版本不匹配或 P0/P1 证据缺失 | 标记 blocked，等待验证轨道完成 | 0 |
...
- **Never skip independent evidence** — required Test Spec and P0/P1 results must be current before `/ship-it`
...
/loop-it
  ...
  ├── evidence gate ← validate Test Spec and independent results
...
```
/prd → /prd-to-spec → approved Spec
  ├── /to-issues (implementation)
  └── /spec-to-test → /to-issues (verification)
      → /loop-it (→ implementation/verification → evidence gate → /review-it → /note-it → /ship-it)×N
```
