...
PRD
  ...
  -> Approved Feature Spec
      ...
      └── spec-to-test -> Approved Test Spec
...
| Skill | 输入 | 输出 | 边界 |
| --- | --- | --- | --- |
| `prd` | 原始需求、产品目标 | 可评审 PRD | 不确定实现架构，不生成 Feature Spec。 |
| `prd-to-spec` | 已接受 PRD | 一个或多个模块化 Feature Spec | 只定义产品与实现契约，不生成独立 Test Spec。 |
| `spec-to-test` | 已批准、已确定版本的 Feature Spec | 与来源版本绑定的 Test Spec | 不从草稿推导发布基线，不读取私有实现细节定义预期行为。 |
| `to-issues` | Approved Feature Spec 或 Approved Test Spec | 实现 Issues 或验证 Issues | 两类 Issue 分开生成，不把实现和独立验证混进同一任务。 |
| `review-it` | 实现结果、测试证据、来源版本 | 评审结论 | Test Spec 失效、版本不匹配或阻塞证据缺失时不得通过。 |
| `note-it` | 已完成的实现与评审 | 实施决策和偏差记录 | 记录发生了什么，不代替 Spec 或评审。 |
| `ship-it` | 已通过评审的交付物 | Commit、PR、合并和 Issue 关闭 | 属于远端变更操作，不得绕过测试和评审关卡。 |

Feature Spec 批准后，实现轨和验证轨可以并行准备：

```text
Approved Feature Spec v1.2
├── 实现轨
│   └── implementation Issues
│       ├── implementation
│       └── implementation-coupled unit tests
└── 验证轨
    └── Test Spec v1.x (source: Feature Spec v1.2)
        ├── API contract
...
测试设计、数据、Mock、API Collection、场景和 k6 模型可以与实现同步准备；依赖可部署目标的真实执行需要等待测试环境就绪。两条轨道最终在测试证据、`review-it` 和 `ship-it` 汇合。Feature Spec 版本变化后，引用旧版本的 Test Spec 必须标记为 `stale`。
...
| 已有代码但没有可信 Spec | `code-to-spec -> 人工校对并批准 Feature Spec -> spec-to-test` |
| 已有 Approved Feature Spec，但缺少独立测试体系 | `spec-to-test -> verification Issues` |
| 已有 Test Spec，需要实施具体测试资产 | `to-issues -> 对应测试执行角色` |

`code-to-spec` 产生的是对当前系统行为的观察结果。代码与产品意图冲突时，需要先处理差异并批准 Feature Spec 基线，不能直接把当前代码当成测试期望。
...
| PRD 到 Spec | 需要解释方案理由、替代方案和兼容性 | `to-design` | 设计提案，供 Feature Spec 采用。 |
| Feature Spec | 领域术语、规则或边界不稳定 | `domain-modeling` | 统一语言、边界和决策记录。 |
| Feature Spec 或实现 | 模块接口、接缝和抽象边界不清 | `codebase-design`、`design-an-interface` | 模块或接口设计选择。 |
...
`loop-it` 用于 Feature Spec、Test Spec 和 Issues 已稳定之后的批量推进：

```text
implementation / verification Issues
  -> loop-it
      ├── 按依赖顺序执行
      ├── 检查 Test Spec 版本和独立验证证据
      ├── review-it
      ├── note-it
      └── ship-it
```

`loop-it` 不生成 PRD、Feature Spec 或 Test Spec，也不能绕过批准、测试和评审。需求仍在变化时，应先返回 `prd` 或 `prd-to-spec`。
...
| `qa-agent` / `deployment-agent` | `review-it`、`ship-it` |
| `execution-editor` | `loop-it`、`note-it` |
...
| [`prd-to-spec`](./prd-to-spec/SKILL.md) | 将 PRD 转换为一个或多个模块化、可版本化的 Feature Spec。 | 需求已确认，需要判断单 Spec 或多 Spec，并形成稳定实现契约。 |
| [`spec-to-test`](./spec-to-test/SKILL.md) | 从 Approved Feature Spec 生成独立、版本绑定的 Test Spec。 | 接口、场景和性能契约已经审查通过，需要准备独立测试体系或为旧项目补测。 |
| [`code-to-spec`](./code-to-spec/SKILL.md) | 从代码、配置和测试反向提取完整 SPEC。 | 接手遗留项目、补齐规格、理解一个缺少文档的现有系统。 |
| [`to-issues`](./to-issues/SKILL.md) | 将 Approved Feature Spec 或 Test Spec 拆成相互隔离的实现或验证 Issue。 | 规格已确认，需要分别进入实现轨道或独立测试轨道。 |
...
| [`note-it`](./note-it/SKILL.md) | 记录实现决策、规格偏差、技术取舍和遗留问题。 | 实现与评审完成后，需要留下可追踪的实施说明。 |
