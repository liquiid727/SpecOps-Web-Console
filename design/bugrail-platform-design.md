...
- Parent integration: Git submodule at `bugrail/`
...
- The product source lives in the `liquiid727/bugrail` fork and is consumed by
  this repository through the pinned `bugrail/` gitlink.
...
- The existing `cli-gui/` product remains a separate, coexisting surface while
  Bugrail is bootstrapped and migrated in explicit phases.
...
- Rewriting the full Codeg UI, Rust runtime, wire identifiers, or SQLite data in
  `BUGRAIL-001`.
- Importing `cli-gui/` implementation or data into Bugrail during bootstrap.
- Replacing upstream history with a source copy or vendored snapshot.
- Treating a clean checkout, a passing local test, or a staged gitlink as release
  evidence.
...
The legacy `Bugrail` theme described in
`.features/product-enhancement-features/spec.md` belongs to the separate
`cli-gui/` surface. It does not define this product's identity or architecture.
...
xintaofei/codeg release tag
          |
          v
liquiid727/bugrail fork history
          |
          v
parent repository gitlink at bugrail/
```

- `bugrail/` is a Git submodule, not copied source and not a parent-repository
  package.
- The parent repository records an exact gitlink commit. The `.gitmodules`
  `branch = main` value is checkout convenience only and never overrides the
  pinned commit.
- The fork uses `origin` for `liquiid727/bugrail` and `upstream` for
  `xintaofei/codeg`.
- Product commits are made in the fork first. The parent gitlink is updated only
  after the fork commit passes its applicable gates.
- A parent change that updates the gitlink records the upstream tag, upstream
  commit, fork integration commit, validation result references, and known
  migration impact.
...
7. Merge and push the validated fork commit, then update the parent gitlink in a
   separate reviewable parent-repository change.
...
## Coexistence With `cli-gui/`

- `cli-gui/` remains the existing standalone Product AI OS CLI workspace
  launcher and retains its own design, Feature Specs, runtime, i18n, and evidence.
- Bugrail and `cli-gui/` do not share package identity, updater channel, app data,
  database schema, local ports, process names, credentials, or release readiness.
- Parent-level specs may compare or coordinate the products, but neither product
  imports the other's implementation during `BUGRAIL-001`.
- A future consolidation requires its own approved Feature/Test Specs and data
  migration plan. Coexistence is the default until that decision exists.
...
| `BUGRAIL-004` Data Migration And Legacy Interop | Define explicit import/coexistence/rollback behavior for existing CodeG and any approved `cli-gui` data. | Deferred; no active Feature Spec. |
...
- provenance: fresh recursive submodule checkout resolves the recorded gitlink,
  exact upstream tag, fork `origin`, and upstream remote;
...
- coexistence: no parent `cli-gui/` files, state, or release claims are changed;
