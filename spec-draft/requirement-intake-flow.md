...
MVP 流程适用于一个项目仓库内的新需求整理、SpecOS Contract 准备、任务计划生成、测试计划生成和测试结果展示。

它承认一个现实：当前系统还没有完整实现 `raw requirement -> SpecOS Contract + Task Plan` 的自动化链路，所以需求进入正式 Change Workspace 前，需要人工或 agent 参与整理。

`spec-web-ui` 不属于单个项目的需求交付主流程。它是一个工具站点和资产工作台，用来维护平时积累的 rules、skills、spec templates、agent templates、workflow templates 和 test patterns。项目可以从这类资产库选择或导入配置，但一个具体需求的 draft、Change Workspace、task-plan、test-plan、result 和 acceptance 仍然应该落在目标项目自己的仓库边界内。
...
原始需求
-> 记录到目标项目的 spec-draft/
-> 人工或 spec-editor agent 整理为 SpecOS Contract 草稿
-> 放入 specs/changes/<change-id>/ Change Workspace
-> 用 CLI 从 SpecOS Contract 生成 task-plan / test-plan / test-schedule
-> 用 test-runner 读取 test-plan 并生成 normalized result
-> 用 test-console 查看结果
-> 人工评审、修正、验收
-> 验收通过后再更新 Project Memory
```
...
| Spec editor agent | 将 draft 整理成 SpecOS Contract |
| Architecture / test agents | 基于 SpecOS Contract 生成 task-plan / test-plan / test-schedule |
...
| SpecOS Contract | `specs/changes/<change-id>/spec.json` | 需要人工或 agent 准备 |
| 测试计划 | `tests/plans/*.test-plan.json` | CLI 已支持从 SpecOS Contract 生成 |
| 任务计划 | `specs/changes/<change-id>/task-plan.md` | CLI 已支持生成 |
...
- Change Workspace 能说明它基于哪个 Project Memory baseline。
- 已生成对应 task-plan、test-plan 和 test-schedule。
...
- draft 到 SpecOS Contract 仍然不是全自动。
...
-> 生成 spec-draft
-> draft review
-> 生成 specs/changes/<change-id>/ Change Workspace
-> 基于 Project Memory + active Change Workspace 做影响分析
-> 生成开发任务、Agent 分工、测试计划、接口/场景资产
-> 执行实现与测试
-> 汇总 evidence result、review notes、acceptance report
-> 验收门禁
-> promote accepted content into Project Memory
-> archive completed change into Evidence Archive
```
...
| Normalize | 把 draft 变成正式 SpecOS Contract | spec.json、change metadata、trace map |
| Plan | 基于 Project Memory + Change Workspace 生成执行计划 | impact analysis、tasks、test-plan、schedule |
| Execute | 执行开发、测试和审查 | code changes、test results、review notes |
| Accept | 验收并更新系统事实 | Project Memory update、Evidence Archive、release notes |
...
- 与 Project Memory baseline 的关系说明。
...
- 更新后的 Project Memory。
- `specs/archive/<change-id>/` Evidence Archive。
...
- 明确 draft 模板。
- 明确 SpecOS Contract 最小结构。
- 明确从 draft 到 SpecOS Contract 的人工 checklist。
...
### 4.2 第二阶段：自动生成 SpecOS Contract

目标：

- 由 spec-editor agent 根据 draft 生成 SpecOS Contract。
...
- Change Workspace 可被 CLI 和 test tooling 消费。

### 4.3 第三阶段：自动生成计划与测试资产

目标：

- 基于 Project Memory + Change Workspace 自动生成 impact analysis。
- 自动生成 task-plan、test-plan、test-schedule、Bruno / Playwright / scenario assets。
...
- 只有实现、测试、review、验收全部通过的 Change Workspace 才能 promote 到 Project Memory。
- Evidence Archive 保留 change 的完整上下文。
- CI gate 能阻止未验收或未对齐的变更进入主线。

判断标准：

- `specs/current/` 是 Project Memory，始终代表已验收事实。
- `specs/changes/` 是 Change Workspace，始终代表进行中的变更。
- `specs/archive/` 是 Evidence Archive，始终保留历史交付依据。

## 5. 后续可转成 spec 的需求草稿

### 5.1 Feature: Requirement Intake Workflow

SpecOS should provide a requirement intake workflow that guides a new requirement from raw input to draft, SpecOS Contract, Task Plan, generated test assets, execution result, and accepted Project Memory update.
...
- Separate Project Memory facts from active Change Workspaces.
...
- The process must produce or reference a SpecOS Contract under `specs/changes/<change-id>/`.
...
- The system should generate a SpecOS Contract from an approved draft.
- The system should compare `specs/current/` and `specs/changes/<change-id>/` to produce impact analysis.
- The system should derive task-plan, test-plan, and test-schedule from the active Change Workspace.
...
- What is the minimum SpecOS Contract schema required for a Change Workspace in the next MVP?
...
- It should not hold the canonical state of `draft -> Change Workspace -> task-plan -> test-plan -> result -> acceptance`.
