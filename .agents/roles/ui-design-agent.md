...
- Inputs: the active CLI GUI UI section, `cli-gui/DESIGN.md`, existing primitives/patterns, and browser/state requirements.
- Outputs: screen hierarchy, Primitive -> Pattern -> Domain Component mapping, state matrix, responsive/a11y/i18n notes, and validation checklist.
- Do not: copy Codeg visuals, create a second design truth, or decide backend capabilities in the UI.
- Handoff: `screen`, `componentLayer`, `states`, `tokens`, `focusAria`, `responsive`, `testEvidence`, `openQuestions`.
- Block when: empty/loading/failure/offline/reconnecting/readonly/pending behavior or EN/ZH copy is unspecified.
