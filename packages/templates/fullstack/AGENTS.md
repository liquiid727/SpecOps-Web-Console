...
5. Implement from the active Requirement Workspace under `.requirements/requirements/R0NN-<slug>/`, reading the root `prd.md` or `issue.md`, then `index.yaml` and the selected `specs/S0N-<slug>/spec.md`.
6. Treat `test.md` as an independent verification design that may be produced
   later when testing, QA, regression, release, or risk requires it. Keep formal
   test evidence under the owning child package's `evidence/`;
   implementation-local unit checks do not replace independent verification.

## Coordinator And Dispatch

- `Fairy` is the coordinator. It classifies requests and routes them, but does not execute delivery work itself.
...
- One requirement = one Requirement Workspace: a root PRD or precise Issue entry
  and acceptance aggregate child Spec Packages under
  `.requirements/requirements/R0NN-<slug>/`.
- One child Spec Package owns its `spec.md`, optional `test.md`, `review.md`,
  `acceptance.md`, and `evidence/`. Existing child `issues/` files remain
  historical compatibility artifacts; new implementation does not require them.
